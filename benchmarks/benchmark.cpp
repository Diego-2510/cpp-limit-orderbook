...
std::vector<OrderInput> make_workload(
    std::size_t count,
    uint32_t first_id,
    uint64_t seed) {
    ...
    std::uniform_int_distribution<uint64_t>
        price_dist(9500, 10500);

    std::uniform_int_distribution<uint32_t>
        quantity_dist(10, 100);
    ...
    for (std::size_t i = 0; i < count; ++i) {
        workload.push_back(
            OrderInput{
                first_id + static_cast<uint32_t>(i),
                side_dist(rng) == 0
                    ? OrderType::BUY
                    : OrderType::SELL,
                price_dist(rng),
...
void run_warmup(
    const std::vector<OrderInput>& workload) {
...
double percentile(
    const std::vector<double>& sorted_ns,
    double p) {
